...
class MSSQLDriverDelegate(private val inboundEventSchema: String, private val outboundEventSchema: String) :
    DriverDelegate {
    ...
    override fun saveIfAbsent(conn: Connection, event: InboundEvent): Boolean {
        val sql = """INSERT INTO $outboundEventSchema 
            | (event_id, occurred_at, type, source_id, source, payload, arrived_at)
            | SELECT ?,?,?,?,?,?,?
            | WHERE NOT EXISTS ( SELECT 1 FROM $outboundEventSchema WHERE event_id = ?)
        """.trimMargin()
